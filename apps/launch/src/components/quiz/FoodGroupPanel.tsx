...
function Chips({ items, tone = "forest" }: { items: string[]; tone?: "forest" | "sage" | "clay" }) {
  if (!items.length) return <p className="text-sm text-forest/55">None in this mix</p>;
  return (
  ...
  return (
    <section aria-labelledby="food-groups-heading" className="rounded-lg border border-forest/12 bg-card p-5 sm:p-7">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-[0.65rem] font-medium tracking-[0.18em] text-forest/60 uppercase">What’s in your week</p>
        ...
        <p aria-live="polite" className="text-sm text-forest/70">
          ...
          {added.length ? <span className="text-sage"> · {added.length} from your sides</span> : null}
        </p>
      </div>

      <div className="mt-6">
        <p className="text-xs font-medium tracking-[0.14em] text-forest/65 uppercase">Vegetable subgroups</p>
          ...
          {VEGETABLE_SUBGROUPS.map((sub) => {
            ...
            return (
              <li key={sub.id} className="min-w-0">
                ...
                <span className={cn("mt-2 flex items-start gap-1 text-[0.7rem] leading-tight", on ? "text-forest" : "text-forest/45")}>
                  {on ? <Check className="mt-px size-3 shrink-0 text-sage" aria-hidden /> : null}
                  <span>
                    {sub.label}
      ...
      <dl className="mt-6 grid gap-5 border-t border-forest/10 pt-5 sm:grid-cols-3">
        <div className="grid content-start gap-2">
          <dt className="text-xs font-medium tracking-[0.14em] text-forest/65 uppercase">Vegetables</dt>
          <dd><Chips items={summary.vegetables} tone="sage" /></dd>
        </div>
        <div className="grid content-start gap-2">
          <dt className="text-xs font-medium tracking-[0.14em] text-forest/65 uppercase">Grains</dt>
        ...
        <div className="grid content-start gap-2">
          <dt className="text-xs font-medium tracking-[0.14em] text-forest/65 uppercase">Protein foods</dt>
          <dd><Chips items={summary.proteins.map((item) => `${item.name} · ${item.source === "plant" ? "plant" : "poultry"}`)} tone="clay" /></dd>
        </div>
      </dl>

      <p className="mt-6 border-t border-dashed border-forest/15 pt-4 text-xs leading-5 text-forest/60">
        ...
        {FOOD_GROUP_SOURCES.map((source, index) => (
          <span key={source.href}>
            {index ? " and " : ""}
            <a href={source.href} target="_blank" rel="noopener noreferrer" className="underline underline-offset-2 hover:text-clay">
              {source.label}
            </a>
          </span>
        ))}
